...
Usage::
    ...
    python create_dambreak_static.py NX NY X_LENGTH Y_WIDTH BED_LENGTH BED_HEIGHT ALFAS T PLOT_FLAG
...
``ALFAS``
    ...
    be computed from this value, assuming a solid density of 2000 kg/m³
...
Example::
    ...
    python create_dambreak_static.py 300 15 3.0 0.15 0.2 0.4 0.6 300 true
...
def usage():
    print("Usage: python create_dambreak_static.py NX NY X_LENGTH Y_WIDTH "
          "BED_LENGTH BED_HEIGHT ALFAS T PLOT_FLAG")
    sys.exit(1)
...
plot_flag = sys.argv[9].lower() == 'true'

# Physical constants (consistent with the Roche example)
n_solid = 1
rho_s = 2000.0              # kg/m^3, solid density
SP_HEAT_S = 1617.0
SP_HEAT_A = 998.0
...
for j in range(ny_cells):
    ...
    P_excess = rho_m * grav * H[j, :]
    # conservative variable for pore pressure: (P_excess) * (rho_m * H)
    q0[-1, :] = P_excess * q0[2, :]
    # Append to file
